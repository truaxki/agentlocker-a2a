#!/usr/bin/env python3
"""
host_agent_card.py — Proposed Fixes v1
Author: Astra (VPS)
Date: 2026-05-28
Reviewed by: IronbornArcher (pending)
Approved by: Kirk (pending)

CHANGES FROM CURRENT host_agent_card.py:
  1. [P0] SingletonLock cleared before every browser launch
  2. [P0] Server upgraded to ThreadingMixIn (handles concurrent requests)
  3. [P1] run_login_prep.py path made relative (no hardcoded /Users/kirk/agentlocker-a2a)
  4. [P1] Basic task state registry added (foundation for RFI loop)
  5. [P1] resume-interactive-session endpoint added (stub, ready for RFI wiring)
  6. [P2] hello_world_secret removed from capabilities
  7. [P2] artifacts_base_url and cdp_mode added to capabilities
  8. [P2] Startup banner with status of key paths

INSTRUCTIONS FOR IRONBORN ARCHER:
  - Review all changes below
  - If no major issues, replace ~/operator-prototype/host_agent_card.py with this file
  - Restart the server: kill existing process, then run:
      cd ~/operator-prototype && .venv/bin/python host_agent_card.py &
  - Confirm server started: curl http://localhost:9999/.well-known/agent-card.json
  - Report back to Kirk/Astra
"""
...
SCRIPT_DIR = Path(__file__).parent  # FIX 3: relative path, works wherever repo is cloned

ARTIFACTS_DIR.mkdir(exist_ok=True, parents=True)
PROFILE_DIR.mkdir(exist_ok=True, parents=True)

# FIX 4: Task state registry for RFI loop
ACTIVE_TASKS = {}  # task_id -> {"state": str, "event": threading.Event, "inputs": dict}
ACTIVE_TASKS_LOCK = threading.Lock()

# FIX 6+7: Clean capabilities — no test artifacts, explicit metadata
CARD_DATA = {
    ...
    "capabilities": {
        ...
        "cdp_mode": False,  # True when connected to real Chrome via --remote-debugging-port=9222
...
def _clear_singleton_lock():
    """FIX 1: Always clear ChromeDriver SingletonLock before launch."""
...
def execute_interactive_login_prep(params):
    ...
    with ACTIVE_LOGIN_PREP_LOCK:
        ...
        script_path = str(SCRIPT_DIR / "run_login_prep.py")  # FIX 3
...
def execute_resume_interactive_session(params):
    """FIX 5: RFI resume handler — unblocks a suspended browser task."""
...
# FIX 2: Threaded server — handles concurrent requests while browser tasks run
